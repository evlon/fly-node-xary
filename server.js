...
const nezha_host = process.env.NEZHA_HOST
const nezha_port = process.env.NEZHA_PORT
const nezha_token = process.env.NEZHA_TOKEN
// const port = 3000;
...
app.get("/nezha", (req, res) => {
  let cmdStr =
    `/bin/bash nezha.sh ${nezha_host} ${nezha_port} ${nezha_token} >/dev/null 2>&1 &`;
  exec(cmdStr, function (err, stdout, stderr) {
    if (err) {
...
app.use(
  ...
  createProxyMiddleware({
    target: `http://127.0.0.1:${port}/`, // 需要跨域处理的请求地址
...
function keepalive() {
  // 1.请求主页，保持唤醒
  let app_url = `https://${process.env.FLY_APP_NAME}.fly.dev`;
  ...
  exec("curl " + app_url + "/status", function (err, stdout, stderr) {
    ...
    if (!err) {
      if (stdout.indexOf("nezha-agent") != -1) {
        ...
        //哪吒未运行，命令行调起
        exec(
          "/bin/bash nezha.sh nez-s.yicdn.vip 5555 aUquPbvIV71UGK0FnF >/dev/null 2>&1 &",
          function (err, stdout, stderr) {
            if (err) {
